...
import { Loader2, UploadCloud, ImageOff, Eye, EyeOff, Save } from "lucide-react";
...
async function updateProfileOnServer(payload: any): Promise<any> {
    // Primary (counselor)
    try {
        return await apiFetch<any>("/counselor/profile", {
            method: "PATCH",
...
function sameProfile(a: ProfileForm | null, b: ProfileForm | null): boolean {
    if (!a || !b) return false;
    return a.name === b.name && a.email === b.email && a.gender === b.gender;
}
...
const CounselorSettings: React.FC = () => {
    ...
    const isCounselorRole = roleNorm.includes("counselor") || roleNorm.includes("counsellor");
    ...
    // Refresh from DB on mount (avoid stale local session values)
    React.useEffect(() => {
    ...
    }, [userId, userName, userEmail, userGender, userRole, userEmailVerifiedAt, userEmailVerified, userVerified]);
    ...
    const existingAvatarUrl =
        user?.avatar_url && typeof user.avatar_url === "string" ? (user.avatar_url as string) : null;
    ...
    const handleSaveProfile = async (event: React.FormEvent) => {
        ...
        if (!isCounselorRole) {
            toast.error("This page is only available for counselor accounts.");
            return;
        }
    ...
    const genderNotInList = !!profileForm.gender && !ALLOWED_GENDERS.has(profileForm.gender);

    return (
        <DashboardLayout
            title="Settings"
            description="Manage your counselor account details, password, and profile picture."
        >
            <div className="flex w-full justify-center">
                <div className="w-full max-w-3xl space-y-6">
                    {/* PROFILE HEADER */}
                    <Card className="border-indigo-100/80 bg-white/80 shadow-sm shadow-indigo-100/60 backdrop-blur">
                        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center">
                            <div className="flex w-full justify-center sm:w-auto sm:justify-start">
                                <Avatar className="h-16 w-16 border border-indigo-100 bg-indigo-50">
                                    {displayAvatarUrl ? (
                                        <AvatarImage
                                        ...
                                        <AvatarFallback className="bg-indigo-100 text-sm font-semibold text-indigo-900">
                                            {initials}
                                        </AvatarFallback>
                                    )}
                                </Avatar>
                            ...
                            <div className="space-y-1 text-center sm:text-left">
                                ...
                                <p className="text-[0.7rem] text-muted-foreground">
                                    Changing your email will require verification to complete the change.
                                </p>
                            </div>
                        </CardHeader>
                    </Card>
                    ...
                    <Card className="border-indigo-100/80 bg-white/80 shadow-sm shadow-indigo-100/60 backdrop-blur">
                        <CardHeader>
                            ...
                            <CardDescription className="text-xs text-muted-foreground">
                                Update your name, email, and gender.
                            </CardDescription>
                        </CardHeader>
                            ...
                            <form className="space-y-4" onSubmit={handleSaveProfile}>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-[0.7rem] text-muted-foreground">
                                        ...
                                        <span className="font-medium">unverified</span> until you verify it.
                                    </p>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    >
                                        <SelectTrigger
                                            id="profile-gender"
                                            className="h-9 w-full text-sm focus:ring-2 focus:ring-indigo-300"
                                        >
                                ...
                                <div className="flex flex-col gap-2 pt-2 sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    <p className="text-[0.7rem] text-muted-foreground">
                                        {profileDirty ? "You have unsaved changes." : "No changes to save."}
                                    </p>
                                </div>
                            </form>
                        </CardContent>
                    ...
                    <Card className="border-indigo-100/80 bg-white/80 shadow-sm shadow-indigo-100/60 backdrop-blur">
                        <CardHeader>
                            ...
                            <CardDescription className="text-xs text-muted-foreground">
                                Upload a clear photo to help students recognize your counselor account.
                            </CardDescription>
                        </CardHeader>
                    ...
                    <Card className="border-indigo-100/80 bg-white/80 shadow-sm shadow-indigo-100/60 backdrop-blur">
                        <CardHeader>
                            ...
                            <CardDescription className="text-xs text-muted-foreground">
                                Use a strong, unique password to protect your counselor account.
                            </CardDescription>
                        </CardHeader>
                        ...
                        <CardContent>
                            <form className="space-y-4" onSubmit={handleChangePassword} autoComplete="off">
                                <div className="space-y-2">
                                    <Label htmlFor="current_password" className="text-xs font-medium text-indigo-900">
                                        Current password
                                    </Label>
                                ...
                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="space-y-2">
                                        <Label htmlFor="new_password" className="text-xs font-medium text-indigo-900">
                                            New password
                                        </Label>
                                        <div className="relative">
                                            <Input
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="confirm_password" className="text-xs font-medium text-indigo-900">

...
type TabKey = "requests" | "assessments";
...
async function fetchCounselingRequests(): Promise<IntakeRequestDto[]> {
    const raw = await counselorIntakeFetch<any>("/counselor/intake/requests");
...
function formatConcernType(raw?: string | null): string {
    ...
    // Fallback: transform "something_like_this" → "Something Like This"
    return value
        .split("_")
...
const CounselorIntake: React.FC = () => {
    ...
    React.useEffect(() => {
        // Load both lists on first visit
        void reloadRequests();
        void reloadAssessments();
    }, [reloadRequests, reloadAssessments]);
    ...
    return (
        <DashboardLayout
            title="Intake"
            description="Review student counseling requests and assessment submissions to support triage and follow-up."
        >
                    ...
                    <div className="flex flex-col gap-3 rounded-lg border border-amber-100 bg-amber-50/70 p-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="space-y-1 text-xs text-amber-900">
                            <p className="font-semibold">
                                Guidance &amp; Counseling – Intake overview
                            </p>
                            <p className="text-[0.7rem] text-amber-900/80">
                                ...
                                and{" "}
                                <span className="font-medium">
                                    mental health needs assessments (Steps 1–3)
                                </span>{" "}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <div className="inline-flex rounded-md border border-amber-200 bg-white/70 p-0.5 text-xs shadow-sm">
                                <button
                                    type="button"
                                    onClick={() => setActiveTab("requests")}
                                    className={`rounded-md px-3 py-1.5 font-medium transition ${activeTab === "requests"
                                ...
                                <button
                                    ...
                                    className={`rounded-md px-3 py-1.5 font-medium transition ${activeTab === "assessments"
                                        ? "bg-amber-600 text-white shadow-sm"
                                        : "text-amber-900 hover:bg-amber-100"
                                        }`}
                                >
                            ...
                            >
                                {(activeTab === "requests" && isLoadingRequests) ||
                                    (activeTab === "assessments" && isLoadingAssessments) ? (
                                    <>
                        ...
                        <Card className="border-amber-100/80 bg-white/80 shadow-sm shadow-amber-100/60 backdrop-blur">
                            <CardHeader className="space-y-1">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    This list shows counseling intake requests submitted by
                                    students, including their main concern, preferred schedule, and
                                    current status.
                                </p>
                            </CardHeader>
                            ...
                            <CardContent className="space-y-3">
                                    ...
                                    <div className="flex items-start gap-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-[0.75rem] text-red-800">
                                        ...
                                        <div>
                                            <p className="font-medium">
                                                Unable to load counseling requests.
                                            </p>
                                ...
                                {!isLoadingRequests && requests.length === 0 && !requestsError && (
                                    <div className="rounded-md border border-dashed border-amber-100 bg-amber-50/60 px-4 py-6 text-center text-xs text-muted-foreground">
                                        No counseling requests have been recorded yet.
                                        <br />
                                        New requests from students will appear here automatically.
                                    </div>
                                )}
                                ...
                                {!isLoadingRequests && requests.length > 0 && (
                                    <div className="space-y-2">
                                        {requests.map((request) => {
                                            const studentName = getStudentDisplayName(request);
                                            const created = formatDateTime(request.created_at);
                                            const scheduleDate = formatDate(
                                                request.preferred_date ?? undefined,
                                            );
                                            const concern = formatConcernType(
                                                request.concern_type ?? undefined,
                                            );
                                            const urgencyLabel = formatUrgency(
                                            ...
                                            return (
                                                <div
                                                    key={request.id}
                                                    className="flex flex-col gap-2 rounded-md border border-amber-50 bg-amber-50/40 px-3 py-2.5 text-xs sm:flex-row sm:items-center sm:justify-between"
                                                >
                                                    {/* Left – student + concern */}
                                                    <div className="space-y-1">
                                                        ...
                                                        {request.details && (
                                                            <p className="max-w-2xl text-[0.7rem] text-muted-foreground">
                                                                {request.details.length > 220
                                                                    ? `${request.details.slice(
                                                                        0,
                                                                        220,
                                                                    )}…`
                                                                    : request.details}
                    ...
                    {activeTab === "assessments" && (
                        <Card className="border-amber-100/80 bg-white/80 shadow-sm shadow-amber-100/60 backdrop-blur">
                            <CardHeader className="space-y-1">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    This list shows consent, demographic snapshots, and mental
                                    health questionnaire responses submitted by students. Scores are
                                    shown for triage and{" "}
                                    <span className="font-medium">
                                        are not diagnostic labels
                                    </span>
                                    .
                                </p>
                            </CardHeader>
                                ...
                                {!isLoadingAssessments &&
                                    ...
                                    !assessmentsError && (
                                        <div className="rounded-md border border-dashed border-amber-100 bg-amber-50/60 px-4 py-6 text-center text-xs text-muted-foreground">
                                            ...
                                            <br />
                                            Once students submit the intake assessment, they will appear
                                            here.
                                        </div>
                                    )}
                                                        ...
                                                        <div className="flex flex-wrap gap-x-3 gap-y-1 text-[0.7rem] text-muted-foreground">
                                                            ...
                                                            <span>
                                                                ...
                                                                {formatLivingSituation(
                                                                    ...
                                                                    assessment.living_situation_other ??
                                                                    undefined,
                                                                )}
                                                            </span>
                                                        </div>
                                                    ...
                                                    <div className="mt-1 flex flex-col items-start gap-1 text-[0.7rem] text-muted-foreground sm:items-end">
                                                        <div className="rounded-md bg-white/90 px-3 py-1.5 shadow-inner shadow-amber-50/70">
                                                            ...
                                                            <p className="mt-1 text-[0.6rem] text-muted-foreground">
                                                                ...
                                                                ; not a diagnosis or clinical label.
                                                            </p>
                                                        </div>

                                                        <p className="max-w-xs text-[0.65rem] text-muted-foreground sm:text-right">
                                                            For full item-by-item responses, please refer
                                                            to the student’s detailed record in the
                                                            Evaluation or Student profile view.
                                                        </p>
                                                    </div>
                                                </div>
                                            );
